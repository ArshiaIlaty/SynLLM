    ...
    try:
        ...
        if 'distribution_metrics' in results:
            ...
            for feature, values in dist_metrics.items():
                if isinstance(values, dict):
                    ...
                    if 'kl_divergence' in values:
                        kl_divergence_values.append(extract_value(values['kl_divergence']))
            ...
            if kl_divergence_values:
                metrics['avg_kl_divergence'] = np.mean([x for x in kl_divergence_values if not np.isnan(x)])
        ...
        if 'numerical_statistics' in results:
            ...
            if mean_diffs:
                metrics['avg_mean_difference'] = np.mean([x for x in mean_diffs if not np.isnan(x)])
        ...
        if 'advanced_metrics' in results and isinstance(results['advanced_metrics'], dict):
            ...
            if 'statistical_tests' in adv_metrics and isinstance(adv_metrics['statistical_tests'], dict):
                ...
                if ks_pvalues:
                    metrics['avg_ks_pvalue'] = np.mean([x for x in ks_pvalues if not np.isnan(x)])
                if ad_stats:
                    metrics['avg_anderson_darling'] = np.mean([x for x in ad_stats if not np.isnan(x)])
                ...
                if isinstance(med_consistency, dict):
                    if 'hba1c_diabetes_relationship' in med_consistency:
                        ...
                        if isinstance(hba1c_rel, dict) and 'real_difference' in hba1c_rel and 'synthetic_difference' in hba1c_rel:
                            ...
                            if real_diff != 0:
                                metrics['hba1c_diff_preservation'] = 1 - abs(real_diff - synth_diff) / abs(real_diff)
                    ...
                    if 'glucose_diabetes_relationship' in med_consistency:
                        ...
                        if isinstance(glucose_rel, dict) and 'real_difference' in glucose_rel and 'synthetic_difference' in glucose_rel:
                            ...
                            if real_diff != 0:
                                metrics['glucose_diff_preservation'] = 1 - abs(real_diff - synth_diff) / abs(real_diff)
            ...
            if 'medical_consistency' in results:
                ...
                if isinstance(med_consistency, dict):
                    if 'glucose_stroke_relationship' in med_consistency:
                        ...
                        if isinstance(glucose_rel, dict) and 'real_means' in glucose_rel and 'synthetic_means' in glucose_rel:
                            real_means = glucose_rel['real_means']
                            synth_means = glucose_rel['synthetic_means']
                            
                            if isinstance(real_means, dict) and isinstance(synth_means, dict):
                                # Get the values for stroke=0 and stroke=1
                                if 0 in real_means and 1 in real_means:
                                    metrics['glucose_real_no_stroke'] = extract_value(real_means[0])
                                    metrics['glucose_real_stroke'] = extract_value(real_means[1])
                                if 0 in synth_means and 1 in synth_means:
                                    metrics['glucose_synth_no_stroke'] = extract_value(synth_means[0])
                                    metrics['glucose_synth_stroke'] = extract_value(synth_means[1])
                    ...
                    if 'age_stroke_relationship' in med_consistency:
                        ...
                        # Add more granular metrics
                        if isinstance(age_rel, dict) and 'real_means' in age_rel and 'synthetic_means' in age_rel:
                            ...
                            if isinstance(real_means, dict) and isinstance(synth_means, dict):
                                # Get the values for stroke=0 and stroke=1
                                if 0 in real_means and 1 in real_means:
                                    metrics['age_real_no_stroke'] = extract_value(real_means[0])
                                    metrics['age_real_stroke'] = extract_value(real_means[1])
                                if 0 in synth_means and 1 in synth_means:
                                    metrics['age_synth_no_stroke'] = extract_value(synth_means[0])
                                    metrics['age_synth_stroke'] = extract_value(synth_means[1])
...
def compare_within_experiment(results_dir, experiment_name, dataset_type='diabetes', output_format='dataframe'):
    ...
    for file in os.listdir(exp_dir):
        if (file.startswith('results_') or 'PROMPT' in file.upper()) and file.endswith('.json'):
    ...
    for file in os.listdir(exp_dir):
        if (file.startswith('results_') or 'PROMPT' in file.upper()) and file.endswith('.txt'):
            prompt_num = extract_prompt_number(file)
            if prompt_num not in processed_prompts:
                result_files.append(file)
                processed_prompts.add(prompt_num)
    ...
    if output_format in ['plot', 'both']:
        ...
        for metric in df.columns:
            if not df[metric].isnull().all() and not df[metric].apply(lambda x: isinstance(x, str)).any():
                try:
            ...
            for col in df.columns:
                try:
                    ...
                    if not numeric_data.isna().all():
                        ...
                        plt.ylabel(col)
                        plt.tight_layout()
                        plot_path = os.path.join(plots_dir, f"{experiment_name}_{col}_comparison.png")
                        plt.savefig(plot_path)
                        plt.close()
...
def compare_across_datasets(results_dirs, prompt_num, dataset_types, output_format='dataframe'):
    ...
    for dataset_name, results_dir in results_dirs.items():
        ...
        print(f"Found {len(exp_dirs)} experiment directories for {dataset_name}: {exp_dirs}")
    ...
    if choice == "1":
        ...
        try:
            ...
            if 0 <= exp_idx < len(exp_dirs):
                compare_within_experiment(results_dir, exp_dirs[exp_idx], dataset, 'both')
            else:
...
if __name__ == "__main__":
    ...
    potential_dirs = {
        'stroke': ['stroke_evaluation_results', 'evaluation_results/stroke', './results/stroke', './stroke_results'],
        'diabetes': ['diabetes_evaluation_results', 'evaluation_results/diabetes', './results/diabetes', './diabetes_results'],
        'cirrhosis': ['cirrhosis_evaluation_results', 'evaluation_results/cirrhosis', './results/cirrhosis', './cirrhosis_results']
    }
    ...
    if len(sys.argv) > 1:
        ...
        if sys.argv[1] == "--help" or sys.argv[1] == "-h":
            ...
            print("  --within DIR EXP    Compare within experiment EXP in directory DIR")
            print("  --across P D1 D2    Compare prompt P across directories D1 and D2")
            print("  --all DIR TYPE      Compare all experiments for dataset TYPE in directory DIR")
        ...
        while True:
            try:
                ...
                cont = input("\nDo you want to continue despite the error? (y/n): ").lower()
                if cont != 'y':
                    print("Exiting...")
                    break
