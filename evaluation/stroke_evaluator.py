...
class StrokeDataEvaluator:
    def __init__(self, real_data, synthetic_data):
        ...
        self.categorical_cols = ["gender", "work_type", "Residence_type", "smoking_status"]
    ...
    def _clean_data(self):
        ...
        for col in self.numerical_cols:
            ...
            self.synthetic_data[col].fillna(self.synthetic_data[col].median(), inplace=True)
    ...
    def calculate_medical_consistency(self):
        ...
        try:
            ...
            consistency_metrics["glucose_stroke_relationship"] = {
                ...
                "difference_in_difference": abs((real_glucose_by_stroke[1] - real_glucose_by_stroke[0]) - 
            ...
            real_hypertension_stroke = pd.crosstab(
                self.real_data["hypertension"], self.real_data["stroke"], normalize="index"
            )
            synth_hypertension_stroke = pd.crosstab(
                self.synthetic_data["hypertension"], self.synthetic_data["stroke"], normalize="index"
            )
            ...
            consistency_metrics["age_stroke_relationship"] = {
                ...
                "difference_in_difference": abs((real_age_by_stroke[1] - real_age_by_stroke[0]) - 
                                              (synth_age_by_stroke[1] - synth_age_by_stroke[0]))
            }
            
            # Check heart_disease and stroke relationship
            real_heart_stroke = pd.crosstab(
                self.real_data["heart_disease"], self.real_data["stroke"], normalize="index"
            )
            synth_heart_stroke = pd.crosstab(
                self.synthetic_data["heart_disease"], self.synthetic_data["stroke"], normalize="index"
            )
        ...
        try:
            ...
            for factor in risk_factors:
                if factor in self.real_data.columns:
                    ...
                    if (self.real_data[factor].isna().all() or 
                        self.synthetic_data[factor].isna().all()):
                        continue
                        
                    real_correlations[factor] = self.real_data[factor].corr(self.real_data["stroke"])
                    synth_correlations[factor] = self.synthetic_data[factor].corr(self.synthetic_data["stroke"])
