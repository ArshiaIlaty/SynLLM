...
class SyntheticDataPromptGenerator:
    ...
    def __init__(
        ...
        if (
            ...
            and len(self.df.select_dtypes(include=["object", "category"]).columns) > 0
        ):
            ...
            for col in self.df.select_dtypes(include=["object", "category"]).columns:
                ...
                ):  # Assuming classification targets have few classes
        ...
        if id_column is None:
            ...
            for col in self.df.columns:
                if col.lower() in ["id", "idx", "index", f"{self.file_name}_id"]:
                    if self.df[col].nunique() == len(
                        self.df
                    ):  # If each value is unique
                        self.id_column = col
        ...
        for col in self.df.select_dtypes(include=np.number).columns:
            if (self.id_column is None or col != self.id_column) and (
                self.target_column is None or col != self.target_column
            ):
    ...
    def generate_column_definitions(self) -> Dict[str, str]:
        ...
        if self.target_column:
            ...
            for val in unique_values:
                ...
                percentage = round(
                    100 * self.df[self.target_column].value_counts(normalize=True)[val],
                    1,
                )
        ...
        for col in self.numerical_columns:
            ...
            if "mean" in clean_name:
                ...
                definitions[
                    col
                ] = f"{'Maximum' if 'max' in clean_name else 'Minimum'} {base_name} value"
            else:
    ...
    # def generate_column_definitions(self, llm_client=None, model_name="facebook/opt-350m"):
    #     """
    #     Use a Hugging Face model to generate column definitions.

    #     Args:
    #         llm_client: The Hugging Face pipeline (if None, will create a new one)
    #         model_name: Name of the Hugging Face model to use
    #     """
    #     # Fall back to basic definitions if no LLM requested
    #     if llm_client is None and model_name is None:
    #         print("Using basic column definitions (no LLM available)")
    #         return self._generate_basic_column_definitions()

    #     # Create a new text generation pipeline if none provided
    #     if llm_client is None:
    #         try:
    #             print(f"Loading Hugging Face model: {model_name}")
    #             llm_client = pipeline("text-generation", model=model_name)
    #             print(f"Model {model_name} loaded successfully")
    #         except Exception as e:
    #             print(f"Error loading model {model_name}: {e}")
    #             print("Falling back to basic column definitions")
    #             return self._generate_basic_column_definitions()

    #     definitions = {}

    #     # Process each column individually to keep prompts manageable
    #     for col in self.df.columns:
    #         # Create a focused prompt for this column
    #         prompt = f"Dataset: {self.file_name.replace('_', ' ').replace('-', ' ')}\n"
    #         prompt += f"Column name: {col}\n"

    #         # Add column metadata
    #         col_type = str(self.df[col].dtype)
    #         unique_count = self.df[col].nunique()
    #         prompt += f"Data type: {col_type}\n"
    ...
    #         # Add context about other column names for better definitions
    ...
    #         prompt += f"Please provide a concise definition (10-15 words) of what the column '{col}' likely represents:"

    #         try:
    #             # Generate text with the pipeline
    #             print(f"Generating definition for column: {col}")
    #             result = llm_client(
    #                 prompt,
    ...
    #             # Extract the definition, handling different output formats
    #             generated_text = ""
    ...
    #             # Extract just the definition part (after the prompt)
    #             if generated_text.startswith(prompt):
    #                 definition = generated_text[len(prompt):].strip()
    #             else:
    #                 definition = generated_text.strip()

    #             # Clean up the definition (take first sentence if multiple)
    ...
    #             print(f"Definition for '{col}': {definition}")

    #         except Exception as e:
    #             print(f"Error generating definition for '{col}': {e}")
    #             # Fallback to basic definition
    #             definitions[col] = self._basic_definition_for_column(col)
        ...
        for col in self.numerical_columns:
            ...
            if self.target_column:
                for class_val in self.df[self.target_column].dropna().unique():
                    ...
                    class_data = self.df[
                        (self.df[self.target_column] == class_val)
                        & (~self.df[col].isna())
                    ][col]
    ...
    def get_example_records(self, n_examples: int = 3) -> List[Dict[str, Any]]:
        ...
        if self.target_column:
            ...
            for i, class_val in enumerate(classes):
                ...
                class_examples = self.df[
                    self.df[self.target_column] == class_val
                ].sample(
                    min(
                        n_from_class,
                        len(self.df[self.df[self.target_column] == class_val]),
                    )
                )
                ...
                if i < j:  # Only get each pair once
                    ...
                    if abs(corr_value) >= threshold:
                        ...
                        strong_correlations.append(
                            (col1, col2, corr_value, direction, strength)
                        )
    ...
    def generate_basic_prompt(self, n_samples: int = 100) -> str:
        ...
        if self.target_column:
            ...
            target_desc = ", ".join(
                [f"'{val}' ({perc}%)" for val, perc in target_dist.items()]
            )
        else:
            target_desc = "N/A"
        ...
        if self.target_column and len(self.numerical_columns) > 0:
            ...
            for col in self.numerical_columns:
                ...
                if len(class_vals) >= 2:  # Need at least two classes
                    ...
                    if (
                        abs(class1_mean - class2_mean)
                        / max(abs(class1_mean), abs(class2_mean))
                        > 0.15
                    ):
            ...
            if significant_cols:
                ...
                if len(significant_cols) == 1:
                    ...
                    prompt += (
                        f"{', '.join(comparisons[:-1])} and {comparisons[-1]}"
        ...
        if self.target_column:
            ...
            if significant_cols:
                significant_cols.sort(
                    key=lambda x: abs(x[1] - x[2]) / max(abs(x[1]), abs(x[2])),
                    reverse=True,
                )
...
"""
        ...
        if strong_correlations:
            ...
            moderate_positive = [
                (c[0], c[1]) for c in strong_correlations if 0.7 <= c[2] <= 0.8
            ]
            moderate_negative = [
                (c[0], c[1]) for c in strong_correlations if -0.8 <= c[2] <= -0.7
            ]
    ...
    def save_prompts_to_files(
        self, n_samples: int = 100, output_dir: str = "./prompts"
    ):
        """
        ...
        return [
            f"{output_dir}/{self.file_name}_{prompt_name}.txt"
...
def main():
    """
    Example usage of the SyntheticDataPromptGenerator class with Hugging Face.
    """
    import argparse

    parser = argparse.ArgumentParser(
        description="Generate synthetic data prompts from a CSV file"
    )
    parser.add_argument("csv_path", type=str, help="Path to the CSV file")
    parser.add_argument(
        "--target", type=str, default=None, help="Name of the target/class column"
    )
    parser.add_argument("--id", type=str, default=None, help="Name of the ID column")
    parser.add_argument(
        "--n_samples", type=int, default=100, help="Number of samples to request"
    )
    parser.add_argument(
        "--output_dir", type=str, default="./prompts", help="Directory to save prompts"
    )
    parser.add_argument(
        "--model",
        type=str,
        default=None,
        help="Hugging Face model to use for definitions",
    )
    parser.add_argument(
        "--fill_missing",
        type=str,
        default=None,
        help="Strategy for missing values (mean, median, mode)",
    )
    args = parser.parse_args()
    # Create the generator
    generator = SyntheticDataPromptGenerator(
        args.csv_path, args.target, args.id, args.fill_missing
    )

    # Prepare HuggingFace model if requested
    # llm_client = None
    # if args.model:
    #     try:
    #         print(f"Loading model: {args.model}")
    #         llm_client = pipeline("text-generation", model=args.model)
    #     except Exception as e:
    #         print(f"Could not load model {args.model}: {e}")
    #         print("Will use basic column definitions instead")

    # # Generate column definitions
    # print("Generating column definitions...")
    # definitions = generator.generate_column_definitions(llm_client, args.model)
    ...
    # Optionally allow manual refinement of definitions
    print("\nAutomatic column definitions:")
    for col, definition in definitions.items():
        print(f"  {col}: {definition}")
