...
class SyntheticDataPromptGenerator:
    ...
    def generate_column_definitions(self, use_llm=True, model_name="distilgpt2"):
        ...
        def get_device():
            ...
            try:
                ...
                if torch.cuda.device_count() > 1:
                    ...
                    print(
                        f"Selected GPU {best_device} with {max_free_memory / 1024**2:.2f} MB free"
                    )
        ...
        try:
            ...
            print(
                f"Processing {len(columns_to_process)} columns for enhanced definitions"
            )
            ...
            for col in columns_to_process:
                ...
                if pd.api.types.is_numeric_dtype(self.df[col]):
                    sample_values = (
                        self.df[col]
                        .dropna()
                        .sample(min(5, len(self.df[col].dropna())))
                        .tolist()
                    )
                    ...
                    sample_values = (
                        self.df[col]
                        .dropna()
                        .sample(min(5, self.df[col].dropna().nunique()))
                        .tolist()
                    )
                ...
                try:
                    ...
                    if prompt in generated_text:
                        ...
                        definition = generated_text[
                            len(generated_text.split("Definition: ")[0])
                            + len("Definition: ") :
                        ].strip()
                    ...
                    if len(definition) >= 5 and len(definition) <= 100:
                        ...
                        print(
                            f"Generated definition for '{col}' was too short or too long, keeping basic definition"
                        )
                ...
                if pd.api.types.is_numeric_dtype(self.df[col]):
                    ...
                    if any(term in clean_name for term in ["mean", "avg", "average"]):
                        base_name = " ".join(
                            [
                                ...
                                if word not in ["mean", "avg", "average"]
                            ]
                        )
                        definitions[col] = f"Mean {base_name} value"
                    elif any(
                        term in clean_name for term in ["std", "stdev", "deviation"]
                    ):
                        base_name = " ".join(
                            [
                                word
                                for word in clean_name.split()
                                if word not in ["std", "stdev", "deviation"]
                            ]
                        )
                        ...
                        base_name = " ".join(
                            [
                                word
                                for word in clean_name.split()
                                if word not in ["ratio", "proportion"]
                            ]
                        )
                        ...
                        base_name = " ".join(
                            [
                                word
                                for word in clean_name.split()
                                if word not in ["count", "number", "num"]
                            ]
                        )
                    ...
                    if value_count <= 10:  # For categorical with few values
                        ...
                        if len(unique_vals) <= 5:  # If few enough to list
                            ...
                            definitions[
                                col
                            ] = f"Categorical variable with values: {vals_str}"
                        else:
                            definitions[
                                col
                            ] = f"Categorical variable with {value_count} unique values"
                    else:
...
def main():
    ...
    parser.add_argument(
        "--cpu_only", action="store_true", help="Force CPU-only mode (no CUDA)"
    )
    ...
    try:
        ...
        if args.model:
            ...
            if args.cpu_only:
                ...
                definitions = generator.generate_column_definitions(
                    use_llm=True, model_name=args.model, force_cpu=True
                )
            else:
                definitions = generator.generate_column_definitions(
                    use_llm=True, model_name=args.model
                )
        else:
        ...
        print(f"Error: {e}")
        print(
            "If this is a parameter error, check that your SyntheticDataPromptGenerator class constructor matches the parameters."
        )
        import traceback
