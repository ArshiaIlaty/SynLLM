...
class CirrhosisDataEvaluator:
    ...
    def calculate_medical_consistency(self):
        ...
        try:
            ...
            real_bilirubin_by_status = self.real_data.groupby("Status")["Bilirubin"].mean()
            synth_bilirubin_by_status = self.synthetic_data.groupby("Status")["Bilirubin"].mean()
            
            # Check if we have the D (death) status to compare
            if 'D' in real_bilirubin_by_status.index and 'D' in synth_bilirubin_by_status.index:
                # Compare D vs C (death vs censored)
                if 'C' in real_bilirubin_by_status.index and 'C' in synth_bilirubin_by_status.index:
                    consistency_metrics["bilirubin_status_relationship"] = {
                        "real_D_vs_C_difference": real_bilirubin_by_status['D'] - real_bilirubin_by_status['C'],
                        "synthetic_D_vs_C_difference": synth_bilirubin_by_status['D'] - synth_bilirubin_by_status['C'],
                    }
            else:
                consistency_metrics["bilirubin_status_relationship"] = {
            ...
            if 'D' in real_age_by_status.index and 'D' in synth_age_by_status.index:
                ...
                if 'C' in real_age_by_status.index and 'C' in synth_age_by_status.index:
                    consistency_metrics["age_status_relationship"] = {
                        "real_D_vs_C_difference": real_age_by_status['D'] - real_age_by_status['C'],
                        "synthetic_D_vs_C_difference": synth_age_by_status['D'] - synth_age_by_status['C'],
                    }
            else:
                consistency_metrics["age_status_relationship"] = {
                    "error": "Missing status groups in data",
    ...
    def calculate_new_metrics(self):
        ...
        try:
            ...
            if "Status" in self.real_data.columns and "Status" in self.synthetic_data.columns:
                metrics["cirrhosis_metrics"] = {}
                
                # Calculate status distribution
                real_status_dist = self.real_data["Status"].value_counts(normalize=True)
                synth_status_dist = self.synthetic_data["Status"].value_counts(normalize=True)
                ...
                metrics["cirrhosis_metrics"]["real_status_distribution"] = real_status_dist.to_dict()
                metrics["cirrhosis_metrics"]["synth_status_distribution"] = synth_status_dist.to_dict()
                ...
                if "Bilirubin" in self.numerical_cols:
                    ...
                    for status in ['C', 'CL', 'D']:
                        if status not in real_bilirubin_by_status:
                            real_bilirubin_by_status[status] = self.real_data["Bilirubin"].mean()
                        if status not in synth_bilirubin_by_status:
                            synth_bilirubin_by_status[status] = self.synthetic_data["Bilirubin"].mean()
                    
                    metrics["cirrhosis_metrics"]["real_bilirubin_by_status"] = real_bilirubin_by_status.to_dict()
                    metrics["cirrhosis_metrics"]["synth_bilirubin_by_status"] = synth_bilirubin_by_status.to_dict()
                
                # Calculate stage distribution if available
                if "Stage" in self.categorical_cols:
                    real_stage_dist = self.real_data["Stage"].value_counts(normalize=True)
                    synth_stage_dist = self.synthetic_data["Stage"].value_counts(normalize=True)
                    
                    metrics["cirrhosis_metrics"]["real_stage_distribution"] = real_stage_dist.to_dict()
                    metrics["cirrhosis_metrics"]["synth_stage_distribution"] = synth_stage_dist.to_dict()
