...
# Configuration
MODEL_NAME = "google/gemma-2b"  # Using Gemma 2B model
# MODEL_NAME = "google/gemma-7b"
OUTPUT_DIR = "gemma-stroke-data-100-5experiments"
...
class GemmaGenerator:
    """Helper class for text generation with Gemma models"""

    def __init__(self, model_name, device="cuda"):
        print(f"Initializing Gemma model: {model_name} on {device}")

        try:
            # Load tokenizer with careful error handling
...
def generate_dataset_for_prompt4(
    ...
    try:
        if prompt_config["use_cpu"] and generator.device == "cuda":
            ...
            cpu_generator = GemmaGenerator(MODEL_NAME, device="cpu")
        else:
...
def generate_dataset(
    ...
    if use_cpu and generator.device == "cuda":
        ...
        try:
            temp_generator = GemmaGenerator(MODEL_NAME, device="cpu")
...
def main():
    ...
    print(f"Starting Gemma model stroke data generation on device {DEVICE}")
    ...
    try:
        ...
        try:
            ...
            generator = GemmaGenerator(MODEL_NAME, device=DEVICE)
            print("Successfully initialized generator")
        except Exception as e:
            print(f"Error initializing Gemma generator: {e}")
            print("Falling back to CPU")
            generator = GemmaGenerator(MODEL_NAME, device="cpu")
