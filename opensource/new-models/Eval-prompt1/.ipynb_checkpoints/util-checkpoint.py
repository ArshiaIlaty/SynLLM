...
# Import the improved DiabetesDataEvaluator class
from fixed_evaluator import DiabetesDataEvaluator

# from one_prompt_eval import DiabetesDataEvaluator

# Define the correct paths
REAL_DATA_PATH = "/home/jovyan/SynLLM/datasets/diabetes_prediction_dataset.csv"
BASE_SYNTHETIC_PATH = "/home/jovyan/SynLLM/opensource/new-models/bash"
PROMPTS = ["prompt1", "prompt2", "prompt3", "prompt4"]
RECORDS_DIRNAME = "records_prompt"
CSV_FILENAME = "diabetes_records.csv"
...
def load_real_data(data_path):
    ...
    try:
        ...
        print(
            f"Successfully loaded real data with {len(real_data)} rows and {len(real_data.columns)} columns"
        )
        return real_data
...
def load_synthetic_data(file_path):
    ...
    try:
        ...
        print(
            f"Successfully loaded synthetic data from {file_path} with {len(data)} rows"
        )
        return data
    except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate synthetic diabetes datasets")
    parser.add_argument(
        "--data-dir",
        type=str,
        default=BASE_SYNTHETIC_PATH,
        help="Directory containing synthetic datasets",
    )
    parser.add_argument(
        "--real-data", type=str, default=REAL_DATA_PATH, help="Path to real dataset"
    )
    parser.add_argument(
        "--output", type=str, default="evaluation_summary.csv", help="Output CSV file"
    )
    parser.add_argument(
        "--records-dir",
        type=str,
        default=RECORDS_DIRNAME,
    ...
    for model_dir in model_dirs:
        ...
        for prompt_name in PROMPTS:
            ...
            records_dir = (
                model_dir / f"{args.records_dir}{prompt_name.replace('prompt', '')}"
            )
            ...
            if not records_dir.exists():
                print(
                    f"⚠️ Skipping: {model_name} × {prompt_name} (directory not found at {records_dir})"
                )
                continue
            ...
            if not file_path.exists():
                print(
                    f"⚠️ Skipping: {model_name} × {prompt_name} (file not found at {file_path})"
                )
                continue
            ...
            for col in [
                ...
                if col not in synthetic_data.columns:
                    print(
                        f"⚠️ Warning: Column '{col}' missing in {model_name} × {prompt_name}"
                    )

            # Run evaluation
            try:
                results = evaluate_model_prompt(
                    real_data, synthetic_data, model_name, prompt_name
                )
                all_results.append(results)
            except Exception as e:
                print(f"❌ Evaluation completely failed: {traceback.format_exc()}")
                all_results.append(
                    {"model": model_name, "prompt": prompt_name, "error": str(e)}
                )
