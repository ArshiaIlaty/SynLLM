    ...
    def _perform_statistical_tests(self):
        ...
        for col in self.numerical_cols:
            try:
                ...
                # Get column data
                real_data = self.real_data[col].values
                synth_data = self.synthetic_data[col].values
                
                # Skip if either dataset is empty or too small
                if len(real_data) < 2 or len(synth_data) < 2:
                    results[col] = {
                        "anderson_darling_statistic": None,
                        "anderson_darling_pvalue": None,
                        "note": "Insufficient data for statistical tests"
                    }
                    continue
                
                # Perform Anderson-Darling test
                try:
                    ad_stat = anderson_ksamp([real_data, synth_data])
                    results[col] = {
                        "anderson_darling_statistic": ad_stat.statistic,
                        "anderson_darling_pvalue": ad_stat.pvalue if hasattr(ad_stat, "pvalue") else None,
                    }
                except Exception as e:
                    results[col] = {
                        "anderson_darling_statistic": None,
                ...
                results[col] = {
                    "anderson_darling_statistic": None,
                    "anderson_darling_pvalue": None,
                    "error": f"Error in statistical tests: {str(e)}"
                }
        
        return results

    def calculate_advanced_metrics(self):
        # Super simplified version that avoids calling any helper methods
        return {
            "numerical_metrics": {},  # Empty placeholder
            "categorical_metrics": {},  # Empty placeholder
            "privacy_metrics": {"nn_distance_ratio": 0.0, "identifiability_score": 0.0},  # Empty placeholder
            "statistical_tests": {},  # Empty placeholder
        }
