...
class StrokeDataEvaluator:
    def __init__(self, real_data, synthetic_data):
        ...
        all_potential_categorical = ["gender", "work_type", "Residence_type", "smoking_status"]
    ...
    def _clean_data(self):
        ...
        for col in self.binary_cols:
            if col in self.real_data.columns:
                ...
                if self.real_data[col].dtype == 'object':
                    self.real_data[col] = self.real_data[col].map({"Yes": 1, "No": 0, 1: 1, 0: 0})
            ...
            if col in self.synthetic_data.columns:
                ...
                if self.synthetic_data[col].dtype == 'object':
                    self.synthetic_data[col] = self.synthetic_data[col].map({"Yes": 1, "No": 0, 1: 1, 0: 0})
        ...
        if 'gender' in self.categorical_cols:
            for df in [self.real_data, self.synthetic_data]:
                if len(df) > 0:
                    if 'Male' not in df['gender'].values and 'Female' not in df['gender'].values:
    ...
    def calculate_medical_consistency(self):
        ...
        try:
            ...
            real_glucose_by_stroke = self.real_data.groupby("stroke")["avg_glucose_level"].mean()
            synth_glucose_by_stroke = self.synthetic_data.groupby("stroke")["avg_glucose_level"].mean()
            
            # Check if both groups (0 and 1) exist in the grouped data
            if 0 in real_glucose_by_stroke.index and 1 in real_glucose_by_stroke.index and \
               0 in synth_glucose_by_stroke.index and 1 in synth_glucose_by_stroke.index:
                consistency_metrics["glucose_stroke_relationship"] = {
                    "real_difference": real_glucose_by_stroke[1] - real_glucose_by_stroke[0],
                    "synthetic_difference": synth_glucose_by_stroke[1] - synth_glucose_by_stroke[0],
                }
            else:
            ...
            if 0 in real_age_by_stroke.index and 1 in real_age_by_stroke.index and \
               0 in synth_age_by_stroke.index and 1 in synth_age_by_stroke.index:
                consistency_metrics["age_stroke_relationship"] = {
                    "real_difference": real_age_by_stroke[1] - real_age_by_stroke[0],
                    "synthetic_difference": synth_age_by_stroke[1] - synth_age_by_stroke[0],
                }
            else:
                consistency_metrics["age_stroke_relationship"] = {
                    "error": "Missing stroke groups in data",
            ...
            if "hypertension" in self.real_data.columns and "hypertension" in self.synthetic_data.columns:
                real_hypertension_by_stroke = self.real_data.groupby("stroke")["hypertension"].mean()
                synth_hypertension_by_stroke = self.synthetic_data.groupby("stroke")["hypertension"].mean()
                
                if 0 in real_hypertension_by_stroke.index and 1 in real_hypertension_by_stroke.index and \
                   0 in synth_hypertension_by_stroke.index and 1 in synth_hypertension_by_stroke.index:
                    consistency_metrics["hypertension_stroke_relationship"] = {
                        "real_difference": real_hypertension_by_stroke[1] - real_hypertension_by_stroke[0],
                        "synthetic_difference": synth_hypertension_by_stroke[1] - synth_hypertension_by_stroke[0],
                    }
    ...
    def calculate_new_metrics(self):
        ...
        try:
            ...
            if "stroke" in self.real_data.columns and "stroke" in self.synthetic_data.columns:
                metrics["stroke_metrics"] = {
                    "real_stroke_rate": float(self.real_data["stroke"].mean()),
                    "synthetic_stroke_rate": float(self.synthetic_data["stroke"].mean()),
                    "rate_ratio": float(self.synthetic_data["stroke"].mean() / self.real_data["stroke"].mean()) 
                        if self.real_data["stroke"].mean() > 0 else None
                }
                ...
                if "age" in self.numerical_cols:
                    ...
                    synth_rates = self.synthetic_data.groupby("age_group")["stroke"].mean()
                    
                    # Store rates in a more accessible format
                    metrics["stroke_metrics"]["real_stroke_rates_by_age"] = real_rates.to_dict()
                    metrics["stroke_metrics"]["synth_stroke_rates_by_age"] = synth_rates.to_dict()
