...
MODEL_CONFIGS = {
    "mistral": {
        ...
        "output_dir": "mistral-stroke-data-100-5experiments",
        "max_new_tokens": 200,
        "prompt_format": "{prompt}",  # Standard format
        "use_flash_attn": True,  # Can use flash attention for efficiency
    },
    "falcon": {
        "name": "tiiuae/falcon-rw-1b",
        "output_dir": "falcon-stroke-data-100-5experiments",
        "max_new_tokens": 200,
        "prompt_format": "{prompt}",  # Standard format
    ...
    "tinyllama": {
        ...
        "output_dir": "tinyllama-stroke-data-100-5experiments",
        "max_new_tokens": 200,
    ...
    "qwen": {
        "name": "Qwen/Qwen-1_8B",
        "output_dir": "qwen-stroke-data-100-5experiments",
        "max_new_tokens": 200,
        "prompt_format": "{prompt}",  # Standard format
        "use_flash_attn": False,
    },
...
parser = argparse.ArgumentParser(
    description="Generate synthetic stroke data using different LLMs"
)
parser.add_argument(
    "--model",
    type=str,
...
parser.add_argument(
    "--num_experiments", type=int, default=5, help="Number of experiments to run"
)
parser.add_argument(
    "--timeout", type=int, default=300, help="Timeout in seconds for generation"
)
parser.add_argument(
    "--memory_fraction", type=float, default=0.6, help="Fraction of GPU memory to use"
)
...
try:
    if args.device == "cuda" and torch.cuda.is_available():
        ...
        # Set to use memory fraction for safety
...
PROMPTS = {
    ...
    "PROMPT_3": {
        ...
        "use_cpu": False,  # Changed to False to use GPU for all prompts
    },
...
class BaseModelGenerator:
    """Base class for text generation with different models"""

    def __init__(self, model_name, device="cuda", use_flash_attn=False):
    ...
    def generate(
        self, prompt, max_new_tokens=200, temperature=0.7, num_return_sequences=1
    ):
        """Base generate method to be implemented by subclasses"""
        raise NotImplementedError("Subclasses must implement the generate method")

    def format_prompt(self, prompt):
        """Format prompt according to model requirements"""
        return PROMPT_FORMAT.format(prompt=prompt)

    def to_cpu(self):
        """Move model to CPU"""
        if self.initialized and hasattr(self, "model") and self.model is not None:
            try:
...
class MistralGenerator(BaseModelGenerator):
    """Helper class for Mistral model generation"""

    def __init__(self, model_name, device="cuda", use_flash_attn=True):
        super().__init__(model_name, device, use_flash_attn)

        try:
            # For Mistral, we need to specifically enable flash attention if available
            attn_implementation = (
                "flash_attention_2" if use_flash_attn and device == "cuda" else "eager"
            )
            ...
            self.model = AutoModelForCausalLM.from_pretrained(
                model_name, trust_remote_code=True, device_map=device, **model_kwargs
            )
    ...
    def generate(
        ...
        try:
            ...
            for output in outputs:
                ...
                generated_texts.append(full_text)  # Keep full text
...
class FalconGenerator(BaseModelGenerator):
    """Helper class for Falcon model generation"""
    ...
    def __init__(self, model_name, device="cuda", use_flash_attn=False):
        ...
        try:
            ...
            print(f"Error initializing Falcon model: {e}")
            if device == "cuda":
                print("Falling back to CPU")
                self.device = "cpu"
                try:
        ...
        try:
            ...
            outputs = self.model.generate(
                **inputs,
                ...
                num_return_sequences=num_return_sequences,
                pad_token_id=self.tokenizer.eos_token_id,
                repetition_penalty=1.2,  # Higher repetition penalty for Falcon
                top_p=0.92,
            )
...
class TinyLlamaGenerator(BaseModelGenerator):
    """Helper class for TinyLlama model generation"""
    ...
    def __init__(self, model_name, device="cuda", use_flash_attn=False):
        ...
        try:
            ...
            print(f"Error initializing TinyLlama model: {e}")
            if device == "cuda":
                print("Falling back to CPU")
                self.device = "cpu"
                try:
    ...
    def generate(
        ...
        try:
            # Format the prompt according to TinyLlama chat template
            formatted_prompt = self.format_prompt(prompt)
            ...
            for output in outputs:
                full_text = self.tokenizer.decode(output, skip_special_tokens=True)
                generated_texts.append(full_text)  # Return full text for processing
...
class QwenGenerator(BaseModelGenerator):
    """Helper class for Qwen model generation"""

    def __init__(self, model_name, device="cuda", use_flash_attn=False):
        super().__init__(model_name, device, use_flash_attn)

        try:
            # Load tokenizer - Qwen requires special handling
            self.tokenizer = AutoTokenizer.from_pretrained(
                model_name, trust_remote_code=True
            )
...
def generate_dataset_for_prompt4(
    ...
    # Create CPU generator for stability if needed
    try:
        if prompt_config["use_cpu"] and generator.device == "cuda":
            print("Creating CPU generator for PROMPT_4")
            model_generator_class = MODEL_GENERATORS[args.model]
            cpu_generator = model_generator_class(
                MODEL_NAME, device="cpu", use_flash_attn=USE_FLASH_ATTN
            )
        else:
    ...
    approaches = [
        # Approach 1: Add a formatting hint without examples
        {
            ...
            12345,Male,65,1,0,Yes,Private,Urban,98.5,27.5,formerly smoked,0
            """,
            "temperature": 0.5,
            "max_tokens": 500,
            "attempts": 5,
        ...
        # Approach 4: Use lower temperature
        {
            ...
            "prompt_addition": """
            Generate exactly these fields with no other text:
            id,gender,age,hypertension,heart_disease,ever_married,work_type,Residence_type,avg_glucose_level,bmi,smoking_status,stroke
            """,
            "temperature": 0.3,
        ...
        # Approach 5: Use higher temperature
        {
            ...
            "prompt_addition": """
            Generate only CSV data with these exact columns:
            id,gender,age,hypertension,heart_disease,ever_married,work_type,Residence_type,avg_glucose_level,bmi,smoking_status,stroke

            Generate at least 10 valid records, one per line.
            """,
            "temperature": 0.9,
            "max_tokens": max_tokens,
            "attempts": 5,
...
def generate_dataset(
    ...
    if use_cpu and generator.device == "cuda":
        ...
        try:
            model_generator_class = MODEL_GENERATORS[args.model]
            temp_generator = model_generator_class(
                MODEL_NAME, device="cpu", use_flash_attn=USE_FLASH_ATTN
            )
    ...
    with open(log_file, "w") as f:
        ...
        f.write(
            f"Parameters: max_tokens={max_tokens}, temperature={temperature}, device={current_generator.device}\n\n"
        )
            ...
            try:
                ...
                if current_generator.device == "cuda":
                    ...
                    try:
                        model_generator_class = MODEL_GENERATORS[args.model]
                        temp_generator = model_generator_class(
                            MODEL_NAME, device="cpu", use_flash_attn=False
                        )
                        current_generator = temp_generator
                    except Exception as e:
                        print(f"Error creating CPU generator: {e}")
...
def main():
    ...
    print(f"Starting {args.model} model stroke data generation on device {DEVICE}")
    ...
    try:
        ...
        try:
            # Initialize the generator based on selected model
            model_generator_class = MODEL_GENERATORS[args.model]
            ...
            print(f"Error initializing {args.model} generator: {e}")
            print("Falling back to CPU")
            model_generator_class = MODEL_GENERATORS[args.model]
            generator = model_generator_class(
                MODEL_NAME, device="cpu", use_flash_attn=False
...
# Script to compare results across models
def compare_models_script():
    """
    Analysis script to compare results across different models
    Run this after generating data with all models
    """
    models = ["mistral", "falcon", "tinyllama", "qwen"]
    output_dirs = [MODEL_CONFIGS[model]["output_dir"] for model in models]

    # Create comparison directory
    comparison_dir = "stroke-model-comparison"
    ...
    for model, output_dir in zip(models, output_dirs):
        print(f"Analyzing results for {model}...")

        # Find all experiment directories
        experiment_dirs = [
            d for d in os.listdir(output_dir) if d.startswith("experiment_")
        ]

        # Track metrics for each prompt
        prompt_metrics = {
            "PROMPT_1": {"times": [], "record_counts": [], "stroke_percents": []},
            "PROMPT_2": {"times": [], "record_counts": [], "stroke_percents": []},
            "PROMPT_3": {"times": [], "record_counts": [], "stroke_percents": []},
            "PROMPT_4": {"times": [], "record_counts": [], "stroke_percents": []},
        }

        # Process each experiment
        for exp_dir in experiment_dirs:
            full_exp_dir = os.path.join(output_dir, exp_dir)

            # Process each prompt's data file
            for prompt in ["PROMPT_1", "PROMPT_2", "PROMPT_3", "PROMPT_4"]:
                data_file = os.path.join(full_exp_dir, f"{prompt}_data.csv")
                ...
                if os.path.exists(data_file):
                    ...
                    # Get record count
                    record_count = len(df)
                    prompt_metrics[prompt]["record_counts"].append(record_count)

                    # Calculate stroke percentage
                    if "stroke" in df.columns:
                        stroke_percent = df["stroke"].mean() * 100
                        prompt_metrics[prompt]["stroke_percents"].append(stroke_percent)

                    # Get timing information from metrics file
                    metrics_file = os.path.join(
                        output_dir,
                        "metrics",
                        f"metrics_{exp_dir.split('_')[1]}_{prompt}.json",
                    )
                    if os.path.exists(metrics_file):
                        with open(metrics_file, "r") as f:
                            metrics_data = json.load(f)
                            prompt_metrics[prompt]["times"].append(
                                metrics_data.get("duration_seconds", 0)
                            )

        # Calculate averages
        for prompt, metrics in prompt_metrics.items():
            avg_time = (
                sum(metrics["times"]) / len(metrics["times"]) if metrics["times"] else 0
            )
            avg_records = (
                sum(metrics["record_counts"]) / len(metrics["record_counts"])
                if metrics["record_counts"]
                else 0
            )
            avg_stroke = (
                sum(metrics["stroke_percents"]) / len(metrics["stroke_percents"])
                if metrics["stroke_percents"]
                else 0
            )

            results[model][prompt] = {
                "avg_generation_time": avg_time,
                "avg_records_generated": avg_records,
                "avg_stroke_percent": avg_stroke,
                "target_stroke_percent": 4.9,
                "stroke_percent_accuracy": 100 - abs(avg_stroke - 4.9) / 4.9 * 100
                if avg_stroke > 0
                else 0,
            }
    ...
    for model in models:
        for prompt in ["PROMPT_1", "PROMPT_2", "PROMPT_3", "PROMPT_4"]:
            metrics = results[model][prompt]
            comparison_rows.append(
                {
                    "Model": model,
                    "Prompt": prompt,
                    "Avg Generation Time (s)": round(
                        metrics.get("avg_generation_time", 0), 2
                    ),
                    "Avg Records Generated": round(
                        metrics.get("avg_records_generated", 0), 2
                    ),
                    "Avg Stroke %": round(metrics.get("avg_stroke_percent", 0), 2),
                    "Target Stroke %": 4.9,
                    "Accuracy %": round(metrics.get("stroke_percent_accuracy", 0), 2),
                }
            )

    # Create DataFrame and save
    comparison_df = pd.DataFrame(comparison_rows)
    comparison_file = os.path.join(comparison_dir, "model_comparison.csv")
    comparison_df.to_csv(comparison_file, index=False)

    # Create summary for each model
    model_summaries = []
    for model in models:
        prompt_results = results[model]
        avg_time = sum(
            p.get("avg_generation_time", 0) for p in prompt_results.values()
        ) / len(prompt_results)
        avg_accuracy = sum(
            p.get("stroke_percent_accuracy", 0) for p in prompt_results.values()
        ) / len(prompt_results)
        ...
        model_summaries.append(
            {
                ...
                "Model Size": "7B" if model == "mistral" else "1B-2B",
                "Notes": "Most accurate"
                if avg_accuracy
                == max(
                    sum(
                        p.get("stroke_percent_accuracy", 0) for p in results[m].values()
                    )
                    / len(results[m])
                    for m in models
                )
                    ...
                    == min(
                        sum(
                            p.get("avg_generation_time", 0) for p in results[m].values()
                        )
                        / len(results[m])
                        for m in models
                    )
...
if __name__ == "__main__":
    try:
        ...
        # Note: Uncomment the line below to run comparison after all models have been generated
