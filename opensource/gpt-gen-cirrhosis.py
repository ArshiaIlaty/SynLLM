...
def clear_memory():
    """Clear GPU and system memory with better error handling"""
    if torch.cuda.is_available():
        try:
            torch.cuda.empty_cache()
            ...
            # If we get a device-side assert, we should switch to CPU
            if "device-side assert" in str(e):
                print("CUDA device-side assert detected during memory clearing")
                # Don't retry CUDA operations here, let the calling function handle it
    gc.collect()
...
def generate_dataset(
    ...
    while len(all_records) < num_records:
        for attempt in range(max_attempts):
            try:
                # Handle previous CUDA assert error by switching to CPU
                if had_cuda_assert and current_device >= 0:
                    print("Switching to CPU due to previous CUDA assert error")
                    try:
                        temp_generator = pipeline(
                            "text-generation",
                            model=MODEL_NAME,
                ...
                try:
                    ...
                    print(f"Warning when clearing memory: {e}")
                    # If error in clearing CUDA memory, try switching to CPU
                ...
                if "device-side assert" in str(e) and current_device >= 0:
                    ...
                    try:
                        ...
                        print(f"Error creating CPU generator: {e2}")
                    continue
                # Handle OOM errors
                elif "out of memory" in str(e) and current_device >= 0:
    ...
    try:
        ...
        if local_device >= 0:
            try:
                ...
                print("Successfully initialized generator with GPU")
            except Exception as e:
