...
# Configuration
MODEL_NAME = "gpt2"
OUTPUT_DIR = "prompt_normal_gpt"
BATCH_SIZE = 5  # Records per generation call
NUM_BATCHES = 20  # 5 * 20 = 100 records
...
def generate_dataset(prompt, num_batches, batch_size):
    """Generate dataset for a specific prompt"""
    all_records = []
    ...
    for batch_idx in range(num_batches):
        try:
            ...
            # Extract only the new generated lines
            new_lines = [line for line in generated.split("\n") if line.count(",") == 8]
            all_records.extend(new_lines[-batch_size:])  # Get most recent valid lines

            print(f"Generated {len(all_records)}/{num_batches*batch_size} records")
...
for prompt_name, prompt_text in PROMPTS.items():
    ...
    dataset = generate_dataset(prompt_text, NUM_BATCHES, BATCH_SIZE)
...
print("\nComparison complete! Results saved in:", os.path.abspath(OUTPUT_DIR))
...
# # Generate in chunks
# batch_size = 50  # Reduce if still crashing
# num_batches = 20
# results = []

# for _ in range(num_batches):
#     output = generator(prompt,
#                       max_length=400,  # Reduced from 1500
#                       num_return_sequences=1,
#                       temperature=0.7,
...
#     generated = output[0]['generated_text']
#     csv_data = generated.split("\n")[-batch_size:]  # Get last generated lines
