...
def validate_record(record):
    ...
    try:
        ...
        if (
            diagnosis.strip() == "M" and values[6] < 0.04
        ):  # concavity_mean too low for M
            return False
                ...
                try:
                    ...
                    if "values" in field:
                        ...
                        if field["name"] == "perimeter_mean" and len(record_values) > 2:
                            ...
                            field["name"] == "perimeter_worst"
                            and len(record_values) > 22
                        ):
                            # perimeter_worst should be correlated with radius_worst
                            radius_worst = float(
                                record_values[22]
                            )  # radius_worst is at index 22
                            # perimeter ≈ 2πr, with some variation
                            variation = random.uniform(0.9, 1.1)  # 10% variation
                        ...
                        if "mean" in field["name"] or "worst" in field["name"]:
                            if field["name"].startswith(
                                ...
                                value = round(
                                    value, 5
                                )  # More decimal places for these measures
                            else:
                                value = round(
                                    value, 2
                                )  # 2 decimal places for other measures
                        elif "se" in field["name"]:
                            value = round(
                                value, 6
                            )  # More precision for standard error values
                except Exception as e:
...
def generate_dataset(
    ...
    if max_tokens > available_tokens:
        print(
            f"Warning: Reducing max_tokens from {max_tokens} to {available_tokens} to fit within model context"
        )
    ...
    with open(log_file, "w") as f:
        ...
        f.write(
            f"Parameters: max_tokens={max_tokens}, temperature={temperature}, device=CPU\n\n"
        )
...
def main():
    ...
    with open(status_file, "w") as f:
        f.write(
            f"Breast cancer data generation experiment started at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        )
...
if __name__ == "__main__":
    try:
        ...
        memory = psutil.virtual_memory()
        print(
            f"Memory: {memory.total / (1024**3):.2f} GB total, {memory.available / (1024**3):.2f} GB available"
        )
