...
MODEL_FAMILY = MODEL_NAME.split("/")[-1].split("-")[0].lower()  # Extract model family name
    ...
    try:
        ...
        try:
            ...
            generator = pipeline(
                "text-generation",
                model=model_name,
                device=-1 if device >= 0 and "out of memory" in str(e).lower() else device,  # Use CPU if GPU OOM
            )
            return generator
        except Exception as e:
        ...
        for record_num in range(needed_records):
            ...
            for field in fields:
                try:
                    ...
                    # Get output text
                    output_text = field_output[0]["generated_text"] if isinstance(field_output, list) else field_output["generated_text"]
