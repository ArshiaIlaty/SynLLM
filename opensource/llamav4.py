...
OUTPUT_DIR = "llama2-7b-100-prompt-in-5experiments"
BATCH_SIZE = 5  # Reduced batch size for Llama-2 which is larger than GPT-2
NUM_BATCHES = 20  # Adjusted to maintain total count
DEVICE = 0 if torch.cuda.is_available() else -1
MAX_NEW_TOKENS = 250  # Increased token generation for Llama-2
...
def validate_record(record):
    ...
    try:
        ...
        if not (4 <= float(hba1c) <= 9):
            return False
        if not (70 <= int(float(glucose)) <= 300):  # Added float conversion for Llama-2
            return False
        ...
        # Debug when needed: print(f"Validation error: {str(e)} on record: {record}")
        return False
...
def generate_dataset(generator, prompt, num_records=100, seed=None):
    ...
    max_attempts = 5  # Increased for Llama-2
    ...
    while len(all_records) < num_records:
        batch_size = min(BATCH_SIZE, num_records - len(all_records))

        for attempt in range(max_attempts):
            try:
                clear_memory()  # Clear memory before generation

                # Generate text with smaller chunks for complex prompts
                ...
                output = generator(
                    ...
                    do_sample=True,
                    top_p=0.95,  # Added top_p for Llama-2
                    top_k=50,  # Added top_k for Llama-2
                    repetition_penalty=1.1,  # Helps prevent repeated patterns
                )
                ...
                existing_records = set(all_records)
                unique_new_records = [
                    r for r in new_records if r not in existing_records
                ]
                ...
                # If we got at least one valid record, consider it a success
                if len(unique_new_records) > 0:
                    break  # Success, break attempt loop
                elif attempt < max_attempts - 1:
                    # No valid records but we still have attempts left
                    print(
                        f"No valid records produced, attempt {attempt + 1}/{max_attempts}"
                    )
                    # Maybe try a different temperature for next attempt
                    generator.model.config.temperature = 0.7 + (0.1 * attempt)
...
def save_and_validate_dataset(records, prompt_name, experiment_num):
    ...
    # Check for potential rule adherence (correlations between factors)
...
def get_system_info():
    ...
    if torch.cuda.is_available():
        try:
            ...
            # Handle case when GPUtil can't access GPU info
            system_info.update({"gpu_info": "Available but details not accessible"})
...
def log_experiment_metrics(
    ...
    metrics = {
        ...
        "records_per_second": (
            num_records / (end_time - start_time) if (end_time - start_time) > 0 else 0
        ),
...
def main():
    # Initialize generator with Llama-2 specific configurations
    print(f"Initializing Llama-2 generator with {MODEL_NAME} on device {DEVICE}")

    try:
        # Attempt to use 8-bit quantization if possible (saves memory)
        generator = pipeline(
            "text-generation",
            model=MODEL_NAME,
            device=DEVICE,
        ...
        print("Using 8-bit quantization for Llama-2")
    except:
        # Fall back to standard 16-bit if 8-bit fails
        print("8-bit quantization not available, using 16-bit")
        generator = pipeline(
            "text-generation",
            model=MODEL_NAME,
            device=DEVICE,
