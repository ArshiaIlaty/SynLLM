...
try:
    ...
    print("Warning: UMAP not available. Some visualization features will be disabled.")
...
class DiabetesDataEvaluator:
    def __init__(self, real_data, synthetic_data):
        ...
        self.mia_evaluator = MIAEvaluator(real_data, synthetic_data)
    ...
    def evaluate_flat(self, model_name, prompt_name):
        """Run all evaluations and return flat dictionary of results."""
        results = {}

        # Run MIA evaluation
        mia_results = self.mia_evaluator.evaluate()
        results.update(
            {
                "mia_black_box_auc": mia_results["black_box_auc"],
                "mia_black_box_accuracy": mia_results["black_box_accuracy"],
                "mia_white_box_auc": mia_results["white_box_auc"],
                "mia_white_box_accuracy": mia_results["white_box_accuracy"],
                "mia_vulnerability_score": mia_results["mia_vulnerability_score"],
            }
        )
        ...
        for col, metrics in dist_metrics.items():
            if isinstance(metrics, dict):
                for metric_name, value in metrics.items():
                    results[f"distribution_metrics_{col}_{metric_name}"] = value

        # Add privacy assessment
        privacy_metrics = self.privacy_risk_assessment()
        results.update(
            {
                "privacy_assessment_overall_privacy_score": privacy_metrics.get(
                    "overall_privacy_score", 0
                ),
                "privacy_assessment_nn_distance_ratio": privacy_metrics.get(
                    "nn_distance_ratio", 0
                ),
                "privacy_assessment_identifiability_score": privacy_metrics.get(
        ...
        # Add utility metrics (TSTR/TRTS)
        utility_metrics = self.calculate_tstr_trts(target_col="diabetes")
        results.update(
            {
                "utility_metrics_TSTR_accuracy": utility_metrics.get(
                    "TSTR_accuracy", 0
                ),
                "utility_metrics_TRTS_accuracy": utility_metrics.get(
                    "TRTS_accuracy", 0
                ),
                "utility_metrics_TSTR_auc_roc": utility_metrics.get("TSTR_auc_roc", 0),
                "utility_metrics_TRTS_auc_roc": utility_metrics.get("TRTS_auc_roc", 0),
            }
        )
    ...
    def privacy_risk_assessment(self, k=5):
        ...
        try:
            ...
            privacy_metrics = {
                # Existing metrics
                "nn_distance_ratio": self._calculate_privacy_metrics().get(
                    "nn_distance_ratio", np.nan
                ),
                "identifiability_score": self._calculate_privacy_metrics().get(
                    "identifiability_score", 0.0
                ),
            }
                ...
                for col in cols:
                    if col in ["age", "bmi"] and col in temp_df.columns:
                        if temp_df[col].dtype in [
                            ...
                            temp_df[col] = pd.qcut(
                                temp_df[col], q=10, duplicates="drop", labels=False
                            )
            ...
            for i in range(1, len(identifying_cols) + 1):
                for cols in itertools.combinations(identifying_cols, i):
                    ...
                    try:
                        ...
                        violation_rate = (
                            records_below_k / len(self.synthetic_data)
            ...
            try:
                ...
                real_sample = self.real_data[self.numerical_cols].sample(
                    sample_size, random_state=42
                )
                synth_sample = self.synthetic_data[self.numerical_cols].sample(
                    sample_size, random_state=42
                )
                ...
                privacy_metrics["similarity_attack_risk"] = {"error": str(e)}
            ...
            try:
                ...
                for _ in range(n_simulations):
                    ...
                    subset_indices = np.random.choice(
                        self.synthetic_data.index, size=subset_size, replace=False
                    )
                    ...
                    for col in self.numerical_cols:
                        ...
                        full_hist, bins = np.histogram(
                            self.synthetic_data[col], bins=20, density=True
                        )
                        subset_hist, _ = np.histogram(
                            subset_data[col], bins=bins, density=True
                        )
                ...
                privacy_metrics["approximate_dp_properties"] = {"error": str(e)}
            ...
            try:
                ...
                if not np.isnan(privacy_metrics["nn_distance_ratio"]):
                    nn_score = min(
                        privacy_metrics["nn_distance_ratio"] / 2, 1.0
                    )  # Cap at 1.0
                    components.append(nn_score)

                # K-anonymity (lower violation rate is better)
                k_anon_score = (
                    1.0 - privacy_metrics["k_anonymity"]["average_violation_rate"]
                )
                components.append(k_anon_score)

                # Similarity attack (higher mean distance is better)
                if (
                    "similarity_attack_risk" in privacy_metrics
                    and "mean_distance" in privacy_metrics["similarity_attack_risk"]
                ):
                    sim_score = min(
                        privacy_metrics["similarity_attack_risk"]["mean_distance"] / 3,
                        1.0,
                    )  # Cap at 1.0
                ...
                if (
                    "approximate_dp_properties" in privacy_metrics
                    and "stability_score"
                    in privacy_metrics["approximate_dp_properties"]
                ):
                    dp_score = privacy_metrics["approximate_dp_properties"][
                        "stability_score"
                    ]
