...
# Configuration
MODEL_NAME = "tiiuae/falcon-7b"  # Can be changed to "mistralai/Mistral-7B-v0.1"
...
GPU_BATCH_SIZE = 2  # How many records to generate per GPU
...
def generate_dataset(
    ...
    """Generate dataset with batching across multiple GPUs"""
    if seed is not None:
        set_seed(seed)
    ...
    while len(all_records) < num_records:
        # Determine number of records to generate in this iteration
        remaining = num_records - len(all_records)

        # If multiple GPUs available, create a smaller batch per GPU
        if num_gpus > 1:
            current_batch_size = min(gpu_batch_size * num_gpus, remaining)
        else:
            ...
            try:
                ...
                if "out of memory" in str(e) and attempt < max_attempts - 1:
                    ...
                    if num_gpus > 1:
                        ...
                        print(f"Permanently reduced GPU batch size to {gpu_batch_size}")
                    else:
...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map="auto",  # Let the library decide how to distribute across GPUs
    )
